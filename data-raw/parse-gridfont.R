...
gridfont       $codepoint <- vapply(gridfont       $char, utf8ToInt, integer(1))
gridfont_smooth$codepoint <- vapply(gridfont_smooth$char, utf8ToInt, integer(1))


colnames(gridfont)        <- c("char", "stroke_idx", "x", "y", "point_idx", "width", "height", 'codepoint')
colnames(gridfont_smooth) <- c("char", "stroke_idx", "x", "y", "point_idx", "width", "height", 'codepoint')


gridfont        <- gridfont       [, c('codepoint', 'stroke_idx', 'point_idx', 'x', 'y', 'width', 'height')]
gridfont_smooth <- gridfont_smooth[, c('codepoint', 'stroke_idx', 'point_idx', 'x', 'y', 'width', 'height')]


int_cols <- c('codepoint', 'stroke_idx', 'point_idx', 'width', 'height')
for (col in int_cols) {
  gridfont       [[col]] <- as.integer(gridfont       [[col]])
  gridfont_smooth[[col]] <- as.integer(gridfont_smooth[[col]])
}
